...
- **config-meta-no-new-modules**：
  - 约束/适用范围：执行 `/config-meta` 时，必须先自行判断应“修正已有规则”还是“删减/新增规则条目”；且除非用户明确提出，否则禁止在 AGENTS.md 中新增一级模块。必须先做根因分析，优先解决机制层面的本质问题，再决定具体改写；若已开启深思模式，必须推理用户潜在意图并识别隐含动作期望，对高概率、低风险、可回滚的动作应直接执行，减少往返沟通。迭代目录按需创建：仅在触达项目代码或非代码内容大量改动时才需要，且默认在改动完成后判定，除非用户明确要求提前处理。
...
- **root-cause-fix-over-band-aid**：
  - 约束/适用范围：修复缺陷时必须先定位深层根因；若问题反映架构边界不清、职责耦合或模型设计缺陷，必须优先给出架构级修正，禁止仅做表层补丁即结束。
