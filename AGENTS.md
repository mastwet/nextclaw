...
- **delete-simplify-before-add**：
  - 约束/适用范围：面对问题修复、功能设计、规则调整、流程治理或架构演进时，默认必须先评估“能否删除”，再评估“能否简化”，只有前两者都不足以解决问题时才允许新增逻辑、抽象、规则、配置或流程；禁止跳过删减/简化评估直接叠加新东西。
  - 示例：发现某流程复杂且易错时，先删除冗余分支、历史兼容路径或重复步骤；若仍需保留该能力，再把多入口收敛为单一路径；只有在删除和收敛后仍存在明确缺口时，才新增最小必要实现。
  - 反例：一看到问题就先加开关、补一层适配、堆一个新模块、添一条新规则或新流程，却不判断旧路径是否可以直接删掉，导致系统持续膨胀。
  - 执行方式：方案评审与实现前必须依次回答三个问题：`能删掉什么？`、`删不掉的部分能如何简化？`、`在删减与简化后还缺什么必须新增？`。若进入新增阶段，必须在结果中说明为何删除和简化不足以解决问题，以及新增为何是最小必要项。
  - 维护责任人：当前助手。
...
- **test-asset-budget-first**：
  - 约束/适用范围：测试默认视为有维护成本的资产，而非零成本堆积物。新增或扩写测试时，必须优先保护稳定外部契约，禁止为易变实现细节、临时重构形态或低价值透传逻辑堆积测试；当测试文件、测试包级体量或 mock 密度进入热点区间时，必须先拆分、合并或降级测试层级，而不是继续加码。
  - 示例：为 CLI/API 返回契约、配置迁移、纯函数业务规则、用户可见回归点补测试；把重 mock 的大入口测试拆成更小的契约测试与纯函数测试；新增测试前明确说明其保护的 contract、风险，以及为何需要这一层级而不是更轻的 smoke / pure function 测试。
  - 反例：只因为“改了点内部结构”就新增一串断言调用顺序的测试；在单个测试文件已很重时继续叠加 case；对简单透传、低价值 getter/setter、纯实现细节写大量 mock 驱动测试；用更多测试去掩盖职责过大的生产代码而不做拆分。
  - 执行方式：新增或修改测试时先回答 `它保护的外部契约是什么`；默认优先保留 smoke、contract、pure function 测试，谨慎增加重 mock 入口测试；使用 `pnpm report:test-governance` 做仓库巡检，必要时用 `pnpm check:test-governance` 启用阻塞模式。治理阈值默认如下：测试文件达到 `300` 行 / `15` 个 case / `5` 个 mock 进入预警，达到 `500` 行 / `25` 个 case / `10` 个 mock 进入阻塞候选；包级测试行数达到 `1000` 且测试/源码比达到 `0.35` 进入预警，达到 `1500` 且比值达到 `0.5` 进入阻塞候选。命中阈值时，优先做拆分、去重、下沉为更轻层级测试，或删除无契约价值的测试。
  - 维护责任人：当前助手。
## Project Rulebook
...
- **file-name-must-match-primary-responsibility**：
  - 约束/适用范围：凡本仓库新增、重命名、移动、拆分，或明确评审源码/脚本/测试文件命名时，必须自动加载并遵循 `file-naming-convention` skill（`.agents/skills/file-naming-convention/SKILL.md`）以及 [`docs/workflows/file-naming-convention.md`](docs/workflows/file-naming-convention.md)；不得仅凭记忆或事后补救处理命名。命名不仅要求格式正确，还要求角色语义准确；当文件名暗示 `cache`、`mapper`、`utils`、`manager` 等角色时，实现必须与该角色的主职责相符。
